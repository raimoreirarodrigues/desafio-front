  ...
  methods: {
    ...
    updateItem() {
       const fetch = async (url) => {
        try {
          this.isLoading = true;
          await axios.put(url,this.agent);
           this.$notify({
                title: "Sucesso",
          ...
          if(error){
             ...
             this.$notify({
              ...
              this.$notify({
                title: "Falha",
                text: "Não foi possível atualizar o representante. Por favor, tente novamente mais tarde.",
                type:'error'
              });
            }
          }
      ...
      fetch(this.url_api+'/agent/'+this.agent.id);
    },
  ...
  beforeCreate(){
    ...
    mounted() {
      this.url_api = process.env.VUE_APP_URL_API
       ...
       this.agent.id = id;
       axios.get(this.url_api+'/agent/'+id).then((response) => {
